...
void findMLP(Float_t phantomSize = 200, Float_t rotation = -1, Float_t spotsize = -1, Float_t trackerDist = -1, Float_t initialEnergy = 230, Int_t material = kWater, Int_t kModelType = kUiB, Bool_t kModelUncertainties = false) {
   ...
   if (trackerDist >= 0) {
      d_entry = trackerDist - 5;
      d_exit = d_exit;
      d_entry_cm = d_entry / 10;
      d_exit_cm = d_exit / 10;
   }
   ...
   else if (trackerDist >= 0) {
      f = new TFile(Form("MC/Output/simpleScanner_energy%.0fMeV_%s_phantom%03.0fmm_tracker%03.0fmm.root", initialEnergy, sMaterial, phantomSize, trackerDist));
      printf("Opening file MC/Output/simpleScanner_energy%.0fMeV_%s_phantom%03.0fmm_tracker%03.0fmm.root\n", initialEnergy, sMaterial, phantomSize, trackerDist);
   }
   ...
   if (spotsize >= 0) {
      ...
      ofstream file(Form("Output/MLPerror_energy%.0fMeV_%s_krah_tracker.csv", initialEnergy, sMaterial), ofstream::out | ofstream::app);
      file << phantomSize << " " << trackerDist << " " <<  gDifferenceNoTrk->Eval(0) << " " << gDifferenceNoTrk->Eval(phantomSize/2) << " " << gDifferenceest->Eval(0) << " " << gDifferenceest->Eval(phantomSize/2) << " " << gDifferenceKrah->Eval(0) << " " << gDifferenceKrah->Eval(phantomSize/2) << " " << hResEnergy->GetMean() << endl;
      file.close();
   }
   else {
