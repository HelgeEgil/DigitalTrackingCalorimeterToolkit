...
for i in `seq -f "%03g" 10 10 330`; do
...
#   divergence=`echo "scale=3; $i/100" | bc`
   ...
   /scratch/gate/gate_v8.1.p01-install/bin/Gate -a "'[energy,230] [dz,$i] [ztracker,$ztracker] [zbeam,$zbeam] [spotsize,3] [divergence,0.02] [emittance,0.15] [rotation,0] [material,Water] [zcalorimeter,$zcalorimeter]'" MLPscanner.mac > terminal_output.txt &
   PIDLIST="$PIDLIST $!"
