...
#   emittance=`echo "scale=3; 3*3 * 3" | bc`
	nice -n 19 cpulimit -l 60 Gate -a "'[energy,230] [dz,160] [ztracker,$ztracker] [zbeam,$zbeam] [spotsize,3] [divergence,3] [emittance,15] [rotation,$i] [material,Water] [zcalorimeter,$zcalorimeter]'" MLPscanner.mac > terminal_output.txt &
   PIDLIST="$PIDLIST $!"
...
#	nice -n 19 cpulimit -l 60 Gate -a "'[energy,230] [dz,$i] [ztracker,$ztracker] [zbeam,$zbeam] [spotsize,3] [divergence,3] [emittance,15] [rotation,0] [material,A150] [zcalorimeter,$zcalorimeter]'" MLPscanner.mac > terminal_output.txt &
#   PIDLIST="$PIDLIST $!"
#   echo "Running with i = $i"
#
