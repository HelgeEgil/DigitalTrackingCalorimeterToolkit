...
XYZVector SplineMLP(Double_t t, XYZVector X0, XYZVector X2, XYZVector P0, XYZVector P2, Double_t Lambda0, Double_t Lambda2) {
   XYZVector P0Lambda, P2Lambda, X2mX0, P2Prime, S, X2Prime;
   Double_t tt = pow(t, 2), ttt = pow(t, 3);

   X2mX0 = X2 - X0;
   Float_t mag = sqrt(X2mX0.Mag2());
   
   P0Lambda = P0 * Lambda0 * mag;
   P2Lambda = P2 * Lambda2 * mag;
...
void findMLP(Double_t phantomSize = 200, Double_t rotation = -1, Double_t spotsize = -1, Double_t initialEnergy = 230, Int_t material = kWater, Int_t kModelType = kUncertain, Bool_t kModelUncertainties = true) {
   ...
   Double_t     sigmaTheta = 0.007; // scattering between last two tracker layers, rad
   Double_t     sigmaPos = 0.008; // Position unc. in tracking layers, mm
   ...
   XYZVector   Xp0, Xp1, Xp2, Xp2prime, Xp3, Xp3prime, X0, X2, X0tps, X0tpsOrig, P0, P0tps, P2, S, P0gaus, scatXp0, scatXp1, P0prime, Xp0prime, Xp1prime, P2gaus, P2prime, scat, scatXp2, scatXp3; // Xp are the plane coordinates, X are the tracker coordinates (X2 = (Xp1 + Xp2) / 2)
   ...
   Double_t     X_mlp_end=0, Y_mlp_end=0, theta_X_mlp_end=0, theta_Y_mlp_end=0;
   Double_t     theta_X_mlp, theta_Y_mlp, X_mlp, Y_mlp, X_mlp_sigma, Y_mlp_sigma;
   ifstream    in;
   TSpline3  * splineMCx, * splineMCy, * splineMLPx, * splineMLPy;
   TSpline3  * splineMLPKrahx, * splineMLPKrahy, * splineCSPKrahx, * splineCSPKrahy;
   ...
   Double_t    differenceSplineZ[10000];
   Double_t    differenceArrayDiff[10000] = {};
   Double_t    differenceArrayDiffKrah[10000] = {};
   Double_t    differenceSplineDiffKrah[10000] = {};
   Double_t    differenceArrayKrahCSP[10000] = {};
   ...
   Double_t    aPosMLPKrahCSPx[10000], aPosMLPKrahCSPy[10000], aPosMLPKrahCSPz[10000];
   ...
   else if (kModelType == kUncertain) {
      sigmaPos = 0.066; // 1 um
      sigmaTheta = 0.01; // 50 mrad
   }
...
//            double s_pos = pow(spotSizeAtX0/10, 2); // cm
            double s_pos = pow(sigmaPos/10, 2);
            ...
            double s_pos_out = pow(sigmaPos / 10, 2); // in cm LLU 66 um, Bergen 5 um
           ...
           // Finished finding optimised version
            ...
            for (Double_t t=0; t<1; t += 0.0025) {
               ...
               if (lastEID < maxAcc) {
                  aPosMLPKrahCSPx[aIdxMLP] = S.X();
                  aPosMLPKrahCSPy[aIdxMLP] = S.Y();
                  aPosMLPKrahCSPz[aIdxMLP] = S.Z();
                  aIdxMLP++;
               }
            }
            ...
            splineCSPKrahx = new TSpline3("splineCSPKrahx", arSplineMLPz, arSplineMLPKrahx, idxSplineMLP);
            splineCSPKrahy = new TSpline3("splineCSPKrahy", arSplineMLPz, arSplineMLPKrahy, idxSplineMLP);
            ...
            if (!isnan(rvalue)) {
               ...
               for (Double_t zSweep = 0; zSweep < phantomSize; zSweep += 0.25) { // Keep Sweep increment high to speed up!
                  ...
                  diff_x = fabs(splineMCx->Eval(zSweep) - splineCSPKrahx->Eval(zSweep));
                  diff_y = fabs(splineMCy->Eval(zSweep) - splineCSPKrahy->Eval(zSweep));
                  differenceSplineDiffKrah[idxDifferenceArray++] += sqrt(pow(diff_x,2) + pow(diff_y,2));
               }
            }
            ...
            delete splineCSPKrahx;
            delete splineCSPKrahy;
         }
   ...
   TGraph *gMLPSchulte = new TGraph(aIdxMLP, aPosMLPKrahCSPz, aPosMLPKrahCSPx); 
   ...
   gMCSchulte->Draw("AP");
   gMLPSchulte->Draw("P");
   gMCSchulte->SetTitle("Double sided - Opt X_{0} + CSP;Depth [mm];X position [mm]");
   
   pad2->cd(2);
   TGraph *gMCSchultey = new TGraph(aIdxMC, aPosMCz, aPosMCy);
   TGraph *gMLPSchultey = new TGraph(aIdxMLP, aPosMLPKrahCSPz, aPosMLPKrahCSPy);
   ...
   gMCSchultey->Draw("AP");
   gMLPSchultey->Draw("P");
   gMCSchultey->SetTitle("Double sided - Opt X_{0} + CSP;Depth [mm];X position [mm]");
   ...
   gMCKrah->Draw("AP");
   gMLPKrah->Draw("P");
   gMCKrah->SetTitle("Double sided - Full extended MLP;Depth [mm];X position [mm]");
   ...
   gMCKrahy->Draw("AP");
   gMLPKrahy->Draw("P");
   gMCKrahy->SetTitle("Double sided - Full extended MLP;Depth [mm];Y position [mm]");
   ...
   TGraph *gDifferenceSplineKrah = new TGraph(idxDifferenceArray, differenceArrayZ, differenceSplineDiffKrah);
   TGraph *gDifferenceKrahCSP = new TGraph(idxDifferenceArray, differenceArrayZ, differenceArrayKrahCSP);

   gDifferenceKrah->SetTitle(";Depth in phantom [mm];Error | MLP - MC | [mm]");
   gDifferencePerfect->SetLineColor(kRed);
   gDifferencePerfect->SetLineWidth(2);
   gDifferenceSplineKrah->SetLineColor(kBlue-4);
   gDifferenceSplineKrah->SetLineWidth(3);
   gDifferenceSplineKrah->SetLineStyle(9);
   gDifferenceKrah->SetLineColor(kBlue-4);
   gDifferenceKrah->SetLineWidth(3);
//   gDifferenceSchulte->Draw("AL");
   gDifferenceKrah->Draw("AL");
   gDifferenceSplineKrah->Draw("L");
//   gDifferencePerfect->Draw("L");
   gDifferenceKrah->GetYaxis()->SetRangeUser(0,2);
   
   TLegend *l = new TLegend(.6, .64, .85, .84);
   l->AddEntry(gDifferenceSplineKrah, "X_{0} Krah + CSP", "L");
   l->AddEntry(gDifferenceKrah, "Krah MLP", "L");
   l->Draw();

   TCanvas *c3 = new TCanvas();
   gDifferenceKrahCSP->SetLineColor(kBlue-4);
   gDifferenceKrahCSP->SetLineWidth(3);
   gDifferenceKrahCSP->SetTitle(";Depth in phantom [mm];Difference full krah vs krah+CSP [um]");
   gDifferenceKrahCSP->Draw("AL");
