...
class Module:
    def __init__(self, name, mother, pos, absorberMaterial, nlayers = 40, firstAbsorberMaterial = kNoMaterial, absorberThickness = 1.5):
        ...
        self.fromZ = 0
        self.thickness = 2*self.glueThickness + self.pcbThickness + self.passiveChipThickness + self.activeChipThickness + self.absorberThickness
    ...
    def makePCB(self):
        name = "PCB"
        size = Size(self.chipSize.dx + self.chipGap.dx, self.chipSize.dy, self.pcbThickness)
        pos = self.getPosDepth(0, self.chipPos.y, size, name)
