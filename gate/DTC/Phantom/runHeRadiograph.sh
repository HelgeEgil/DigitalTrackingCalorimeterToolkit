...
for x in `seq -f "%04.f" -$xlim $step $xlim`; do
   for y in `seq -f "%04.f" -$ylim $step $ylim`; do
     ...
     tsp /build/gate/Gate-8.2reducedRootOutput-install/bin/Gate -a "'[phantom,$phantom] [spotx,$x] [spoty,$y] [theta,$theta] [axisx,$axis_x] [axisy,$axis_y] [rotation,90]'" Main_He_phantom.mac
      PIDLIST="$PIDLIST $!"
