...
for rot in `seq -f "%03.0f" 4 4 356`; do
   ...
   for x in `seq -f "%04.f" -84 7 84`; do
      for y in `seq -f "%04.f" -28 7 28`; do
        ...
        tsp /build/gate/Gate-8.2reducedRootOutput-install/bin/Gate -a "'[phantom,$phantom] [rotation,$rot] [spotx,$x] [spoty,$y] [theta,$theta] [axisx,$axis_x] [axisy,$axis_y]'" Main_He_phantom.mac
      done
   done
