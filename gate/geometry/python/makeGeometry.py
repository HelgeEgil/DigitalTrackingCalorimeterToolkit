...
class Module:
    ...
    def addRotation(self):
        ...
        self.objects.append(Rotation(name, filename, rotation, translationA, translationB))
        print("\033[1mAdded rotation of {} with {} degrees and {} mm +- translation\033[0m".format(name, rotation, translationA))
    ...
    def plotGeometry(self, fig = None):
        ...
        for rot in self.objects:
            if rot.type == "Rotation":
                for obj in self.objects:
                    if obj.type == "Box" and obj.name[0:-1] != "Absorber":
                        pos = obj.pos
                        size = obj.size
...
#        self.firstmodule = firstmodule
    ...
    def show_geometry(self):
        self.readyModule()
        figure = self.module.plotGeometry()
        figure = self.firstmodule.plotGeometry(figure)
...
def main():
    ...
    # Re-edit to insert all first-modules
    ...
    module = Module("Module", DTC.name, Pos(0,0,0), kTungsten, 40)
    
    root = Tk()
    mainmenu = MainMenu(root, firstmodule)
    root.mainloop()
