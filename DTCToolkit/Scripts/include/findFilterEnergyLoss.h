...
#ifndef findFilterEnergyLoss_h
#define findFilterEnergyLoss_h
...
class TH2;
Int_t run_energy;
Float_t run_thickness;

// Header file for the classes stored in the TTree if any.

// Fixed size dimensions of array or collections stored in the TTree if any.

class findFilterEnergyLoss {
public :
...
// used to generate this class and read the Tree.
   if (tree == 0) {
      ...
      chain->Add(Form("Data/MonteCarlo/test_scintillatorfilter_%dMeV.root/Hits", energy));
      tree = chain;
#endif // SINGLE_TREE
...
findFilterEnergyLoss::~findFilterEnergyLoss()
{
   if (!fChain) return;
   delete fChain->GetCurrentFile();
}

Int_t findFilterEnergyLoss::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t findFilterEnergyLoss::LoadTree(Long64_t entry)
{
...
void findFilterEnergyLoss::Init(TTree *tree)
{
...
Bool_t findFilterEnergyLoss::Notify()
{
...
void findFilterEnergyLoss::Show(Long64_t entry)
{
...
Int_t findFilterEnergyLoss::Cut(Long64_t entry)
{
...
#endif // #ifdef findFilterEnergyLoss_cxx
